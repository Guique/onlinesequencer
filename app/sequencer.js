...
function updateKeys() {
  for (var i = 0; i < keyTextElements.length; i++) {
    if (settings['numNotes'] - i - 1 >= settings['min'][instrument]
        && settings['numNotes'] - i - 1 <= settings['max'][instrument]) {
      if (instrument == 2) {
...
function playNote(instrument, name, length, delay) {
  ...
  if (settings['numNotes'] - idx - 1 >= settings['min'][instrument]
      && settings['numNotes'] - idx - 1 <= settings['max'][instrument]) {
    audioSystem.play(
        instrument,
        (settings['numNotes'] - (idx + settings['offset'][instrument]) - 1),
        delay);
  }
...
function create() {
  ...
  for (var i = 0; i < settings['instruments'].length; i++) {
    ...
    mainStylesheet.insertRule(
        '.instrument' + i + ' {cursor: pointer; z-index: 96;}',
  ...
  instrumentSelect.onchange = function () {
    ...
    for (var i = 0; i < selectedNotes.length; i++) {
      song.moveNote(
          selectedNotes[i],
          selectedNotes[i].instrument,
          instrument,
          selectedNotes[i].time,
          selectedNotes[i].time,
          selectedNotes[i].type,
          selectedNotes[i].type);
  ...
  instrumentSelect.onmousedown = keySelect.onmousedown = function () {
    selectOpen = true
  };
...
function onNoteClick(event) {
  ...
  if (element != undefined&& element.noteData != undefined
      && !instrumentLock[element.noteData.instrument]) {
...
$(document).on('paste', 'body', function (e) {
  ...
  if (text) {
    ...
    if (parts.length == 3 && parts[0] == settings['clipboardMagicString']
        && parseInt(parts[1]) != instanceId) {
      clearSelection();
...
function updateWavesurferWidth(redraw) {
  if (audioSystem.audioTrack != null) {
    ...
    $('canvas')
        .attr('style', 'position: absolute; z-index: 1337; width: ' + displayWidth + 'px !important; height: 32px;');
